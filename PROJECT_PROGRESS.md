...
- Phase 6 后半：23 题全量评测（需要 gold 对比器做成功判定）+ 完整消融表（4 项消融数据已备：检索多样性/Validator/记忆/模型路由待做）
- 模型路由消融（便宜模型做初筛，强模型做规划/综合——端点有 qwen-turbo/plus/max 可用）
- Phase 7：工程化（README、架构图、简历材料）
